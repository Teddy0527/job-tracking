...
## 1. Supabase SQL Editorで以下のSQLを実行してください:
...
移行が正常に完了したら、一時的な修正コードを削除してください:

1. `src/services/supabase.ts`の`createCompany`関数から以下の行を削除:
   ```javascript
   // 一時的な修正: データベース移行まで既存カラムに空値を設定
   industry: '',
   position: ''
   ```
...
ブラウザで以下のURLを開いてください:
https://app.supabase.com/project/zmfecumkmkaajbeeqoso/sql/new

上記のSQLをコピー&ペーストして実行してください。